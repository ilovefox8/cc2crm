...
class Route
{
    ...
    protected $paths = array(
        'corePath' => 'application/Fox/Resources/routes.json',
        'modulePath' => 'application/Fox/Modules/{*}/Resources/routes.json',
        'customPath' => 'custom/Fox/Custom/Resources/routes.json',
    );
    ...
    protected function init()
    {
        if (file_exists($this->cacheFile) && $this->getConfig()->get('useCache')) {
            ...
            if ($this->getConfig()->get('useCache')) {
                ...
                if ($result == false) {
                    throw new \Fox\Core\Exceptions\Error('Route - Cannot save unified routes');
                }
            }
        }
    }
