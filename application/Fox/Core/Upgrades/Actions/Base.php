...
abstract class Base
{
    ...
    public function __construct(\Fox\Core\Container $container, \Fox\Core\Upgrades\ActionManager $actionManager)
    {
        ...
        $this->zipUtil = new \Fox\Core\Utils\File\ZipArchive($container->get('fileManager'));
    }
    ...
    /**
     * Check if version of upgrade/extension is acceptable to current version of FoxCRM
     *
    ...
    protected function isAcceptable()
    {
        ...
        if (isset($manifest['acceptableVersions'])) {
            $res &= $this->checkVersions($manifest['acceptableVersions'], $this->getConfig()->get('version'), 'Your FoxCRM version doesn\'t match for this installation package.');
        }
